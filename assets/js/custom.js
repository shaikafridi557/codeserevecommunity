...
function loadCSS(pageName) {
    const cssFileName = pageName.toLowerCase() + '-style.css';
    const cssPath = `assets/css/${cssFileName}`;

    const existingLink = document.querySelector(`link[href*="${cssFileName}"]`);
    if (existingLink) {
        console.log(`${cssFileName} is already loaded.`);
    ...
    return new Promise((resolve, reject) => {
        ...
        link.href = cssPath;
        link.onload = () => {
            console.log(`${cssFileName} loaded successfully.`);
            resolve();
        };
        link.onerror = () => {
            console.error(`Error loading ${cssFileName}.`);
            reject();
        };
...
(function ($) {
  ...
  function dsnAjax() {
    return dsnGrid.dsnAjax({
      ...
      async success(data) {
      // Extract page title from the loaded HTML data
      const parser = new DOMParser();
      const doc = parser.parseFromString(data, 'text/html'); // data is the full HTML string
      ...
      if (cssToLoad) {
          await loadCSS(cssToLoad).catch(err => console.error(err));
      }
